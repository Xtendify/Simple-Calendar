...
class System_Status extends Admin_Page
{
  ...
  foreach ($active_plugins as $plugin) {
  	...
  	if (!empty($plugin_data["Name"])) {
  		...
  		$sections["plugins"][sanitize_key(strip_tags($plugin_name))] = [
  			"label" => $plugin_name,
  ...
  // WP Remote POST test.
  $response = wp_safe_remote_post("https://www.paypal.com/cgi-bin/webscr", [
  	"timeout" => 60,
  	"body" => [
  ...
  $sections["server"] = [
  	...
  	"curl_init" => [
  		"label" => "cURL",
  		"result" => !empty($curl_info)
  			? $curl_info["version"] . ", " . $curl_info["ssl_version"]
  			: __("No version found.", "google-calendar-events"),
  		"result_export" => !empty($curl_info)
  			? $curl_info["version"] . ", " . $curl_info["ssl_version"]
  			: "No version found.",
  	],
  	"soap" => [
