...
if (defined('SIMPLE_CALENDAR_GOOGLE_PRO_VERSION')) {
	...
	foreach ($active_plugins as $p) {
		...
		if (strpos($p_lower, 'google-calendar-pro') !== false || strpos($p_lower, 'simple-calendar-google-calendar-pro') !== false) {
			$is_pro_active = true;
			break;
		}
	}
...
$google_pro = isset($feeds_options['google-pro']) && is_array($feeds_options['google-pro']) ? $feeds_options['google-pro'] : [];
...
$should_hide_progress = $has_published_calendar && $completed_timestamp > 0 && time() - $completed_timestamp >= $hide_progress_after;
