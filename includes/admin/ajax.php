...
class Ajax
{
	...
	public function __construct()
	{
		...
		add_action('wp_ajax_simcal_date_i18n_input_preview', [
			$this,
			'date_i18n',
		]);

		// Manage an add-on license activation or deactivation.
		add_action('wp_ajax_simcal_manage_add_on_license', [
			$this,
			'manage_add_on_license',
		]);

		// Reset add-ons licenses.
		add_action('wp_ajax_simcal_reset_add_ons_licenses', [
			$this,
			'reset_licenses',
		]);
	}
	...
	{
		update_option(
			'simple-calendar_admin_footer_text_rated',
			date('Y-m-d', time())
		);
	}
	...
	{
		$addon = isset($_POST['add_on'])
			? sanitize_key($_POST['add_on'])
			: false;
		$action = isset($_POST['license_action'])
			? esc_attr($_POST['license_action'])
			: false;
		$key = isset($_POST['license_key'])
			? esc_attr($_POST['license_key'])
			: '';
		...
		if (
			false === $addon ||
			!in_array($action, ['activate_license', 'deactivate_license'])
		) {
			wp_send_json_error(
				__(
					'Add-on unspecified or invalid action.',
		...
		if (!wp_verify_nonce($nonce, 'simcal_license_manager')) {
			wp_send_json_error(
				sprintf(
					__('An error occurred: %s', 'google-calendar-events'),
		...
		$response = wp_remote_post(
			defined('SIMPLE_CALENDAR_STORE_URL')
				? SIMPLE_CALENDAR_STORE_URL
				: simcal_get_url('home'),
			[
				'timeout' => 15,
				'sslverify' => false,
		...
		if (is_wp_error($response)) {
			wp_send_json_error(
				sprintf(
					__(
						'There was an error processing your request: %s',
						'google-calendar-events'
					),
	...
	public function reset_licenses()
	{
		...
		if (
			empty($nonce) ||
			!wp_verify_nonce($nonce, 'simcal_license_manager')
		) {
			wp_send_json_error(
				sprintf(
					__('An error occurred: %s', 'google-calendar-events'),
