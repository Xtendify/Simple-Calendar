	...
	public function __construct($posts)
	{
		if (!empty($posts) && is_array($posts)) {
			foreach ($posts as $post) {
				$gce_list_max_num = get_post_meta(
					$post->ID,
					"gce_list_max_num",
					true
				);
				$gce_list_max_length = get_post_meta(
					$post->ID,
					"gce_list_max_length",
					true
				);
				$gce_feed_start_interval = get_post_meta(
					$post->ID,
					"gce_feed_start_interval",
					true
				);
				$gce_feed_start = get_post_meta(
					$post->ID,
					"gce_feed_start",
					true
				);
				$gce_feed_end_interval = get_post_meta(
					$post->ID,
