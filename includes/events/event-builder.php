...
class Event_Builder
{
		...
		if ($calendar instanceof Calendar && $event instanceof Event) {
			switch ($tag) {
				...
				case 'end-location':
					...
					$meta_location_name_and_address = empty($location)
	...
	private function get_when(Event $event)
	{
		...
		if (!$event->whole_day) {
			...
			if ($end instanceof Carbon) {
				$time_end =
					'<span class="simcal-event-end simcal-event-end-time" ' .
					'data-event-end="' .
					$end_ts .
					'" ' .
					'data-event-format="' .
		...
		if ($event->multiple_days) {
			$output =
				'<span class="simcal-event-start simcal-event-start-date" ' .
				'data-event-start="' .
				$start_ts .
				'" ' .
				'data-event-format="' .
	...
	public function get_schema_meta()
	{
		...
		if ($event->end_dt instanceof Carbon) {
			$html .=
				'<meta itemprop="endDate" content="' . esc_attr($event->end_dt->toIso8601String()) . '" />';
		}
		...
		$html .=
			...
			esc_attr($this->get_schema_attendance_mode()) .
			'" />';
	...
	private function get_schema_performer_meta()
	{
		$organizer = $this->event->get_organizer();
		if (!empty($organizer) && is_array($organizer) && !empty($organizer['name'])) {
			return '<span itemprop="performer" itemscope itemtype="https://schema.org/Person" style="display:none;">' .
				'<meta itemprop="name" content="' .
				esc_attr($organizer['name']) .
				'" />' .
				'</span>';
		}
		...
		return '<span itemprop="performer" itemscope itemtype="https://schema.org/Organization" style="display:none;">' .
			'<meta itemprop="name" content="' .
			esc_attr($site_name) .
			'" />' .
			'</span>';
	 ...
	 *
	 * Calendar events are treated as free listings unless ticket data exists.
	 *
	 * @since  4.1.1
	 * @access private
	 *
	...
	{
		$offer_url = !empty($this->event->link) ? $this->event->link : '';
		if (empty($offer_url)) {
			$offer_url = home_url(add_query_arg([]));
		}
		...
		$html .= '<meta itemprop="url" content="' . esc_url($offer_url) . '" />';
		$html .= '<meta itemprop="price" content="0" />';
		$html .= '<meta itemprop="priceCurrency" content="' . esc_attr($currency) . '" />';
		$html .= '<meta itemprop="availability" content="https://schema.org/InStock" />';
		if (!empty($valid_from)) {
			$html .= '<meta itemprop="validFrom" content="' . esc_attr($valid_from) . '" />';
		}
		$html .= '</span>';
	...
	{
		$array = apply_filters('simcal_event_tags_add_custom', []);
